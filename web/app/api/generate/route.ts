...
export async function POST(req: Request) {
  ...
  const lang = asLang(String(form.get("lang") ?? "").trim());
  if (!company || !url) {
    return new NextResponse(lang === "en" ? "Company name and URL are required." : "회사명과 URL은 필수입니다.", { status: 400 });
  }
  ...
  const truncMark = lang === "en" ? "\n…(truncated)" : "\n…(이하 생략)";
  if (notes.length > MAX_NOTES_CHARS) notes = notes.slice(0, MAX_NOTES_CHARS) + truncMark;
  ...
  return NextResponse.json({
    ...
    effort,
    key_source: apiKey ? (lang === "en" ? "custom" : "설정") : HAS_API_KEY ? ".env" : "none",
    attached,
    skipped,
  });
}
